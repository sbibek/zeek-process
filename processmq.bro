...
event tcp_packet(c:connection, is_orig: bool, flags:string, seq:count, ack:count, len:count, payload: string){
	...
	# Its observed that the is_orig flag can change according to the packet captured. It depends on if the inital handhsake was captured
	# so make sure to put proper T/F accoriding to the capture. 
	if(flags == "AP" && is_orig == T){
# 	print c$id$orig_h, c$id$orig_p,c$id$resp_h, c$id$resp_p, is_orig ;
